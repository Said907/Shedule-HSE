...
def test_ask_course(message_mock):
...
def test_ask_group(message_mock):
...
def test_handle_start(message_mock):
...
def test_handle_messages(message_mock):
...
def test_ask_for_note(message_mock):
...
def test_save_note(message_mock):
...
def test_save_note_to_database(message_mock):
    with patch('schedule.sqlite3.connect') as mock_connect:
        with patch('schedule.sqlite3.Cursor') as mock_cursor:
            with patch('schedule.bot.send_message') as mock_send_message:
...
def test_show_notes(message_mock):
...
def test_ask_combined_filter_params(message_mock):
    with patch('schedule.bot.send_message') as mock_send_message:
        with patch('schedule.bot.register_next_step_handler') as mock_register_next_step_handler:
            ...
            mock_send_message.assert_called_with(
                message_mock.from_user.id,
                'Введите параметры для комбинированного фильтра (через запятую): Преподаватель,Предмет,Дата,'
                'Тип занятий (введите от 2 до 4 параметров)'
            )

            mock_register_next_step_handler.assert_called_with(message_mock, process_combined_filter_params)


def test_process_combined_filter_params(message_mock):
                ...
                with patch('schedule.get_all_lesson_types_subject') as mock_get_all_lesson_types_subject:
                    with patch('schedule.get_all_lesson_types_date') as mock_get_all_lesson_types_date:
                        with patch('schedule.filter_lessons_by_type') as mock_filter_lessons_by_type:
                            ...
                            mock_send_message.assert_called_with(
                                message_mock.from_user.id,
                                'Отфильтрованные данные по типу занятий "Преподаватель: Иванов, Предмет: Математика, '
                                'Дата: 2023.01.01, Тип занятий: Лекция":\n'
                                '-----------------------------------------------------\n'
                                '...'
                            )
...
def test_ask_for_day(message_mock):
    with patch('schedule.types.ReplyKeyboardMarkup') as mock_reply_keyboard_markup:
        with patch('schedule.types.KeyboardButton') as mock_keyboard_button:
            with patch('schedule.bot.send_message') as mock_send_message:
                with patch('schedule.bot.register_next_step_handler') as mock_register_next_step_handler:
...
def test_get_selected_day(message_mock):
...
def test_show_subjects(message_mock):
...
def test_show_filter_keyboard(message_mock):
    with patch('schedule.types.ReplyKeyboardMarkup') as mock_reply_keyboard_markup:
        with patch('schedule.types.KeyboardButton') as mock_keyboard_button:
            with patch('schedule.bot.send_message') as mock_send_message:
...
def test_ask_for_lesson_id_filter(message_mock):
...
def test_filter_lessons_by_id(message_mock):
...
def test_show_subject_filter(message_mock):
    with patch('schedule.types.ReplyKeyboardRemove') as mock_reply_keyboard_remove:
        with patch('schedule.bot.send_message') as mock_send_message:
            with patch('schedule.bot.register_next_step_handler') as mock_register_next_step_handler:
...
def test_handle_subject_filter_lecturer(message_mock):
...
def test_show_lecturer_filter(message_mock):
    with patch('schedule.types.ReplyKeyboardRemove') as mock_reply_keyboard_remove:
        with patch('schedule.bot.send_message') as mock_send_message:
            with patch('schedule.bot.register_next_step_handler') as mock_register_next_step_handler:
...
def test_handle_subject_filter_subject(message_mock):
...
def test_show_date_filter(message_mock):
    with patch('schedule.types.ReplyKeyboardMarkup') as mock_reply_keyboard_markup:
        with patch('schedule.types.KeyboardButton') as mock_keyboard_button:
            with patch('schedule.bot.send_message') as mock_send_message:
                with patch('schedule.bot.register_next_step_handler') as mock_register_next_step_handler:
...
def test_process_date_filter_response(message_mock):
...
def test_show_lesson_type_filter(message_mock):
    with patch('schedule.types.ReplyKeyboardMarkup') as mock_reply_keyboard_markup:
        with patch('schedule.types.KeyboardButton') as mock_keyboard_button:
            with patch('schedule.bot.send_message') as mock_send_message:
                with patch('schedule.bot.register_next_step_handler') as mock_register_next_step_handler:
...
def test_process_lesson_type_choices(message_mock):
...
def test_answerempty(message_mock):
    with patch('schedule.bot.send_message') as mock_send_message:
...
def test_send_filtered_lessons(message_mock):
    filtered_lessons = [
